...
describe('DeliveryCheckOut', () => {
  ...
  it('should be able to update the end date of delivery', async () => {
    ...
    const updatedDeliveryStatus = await request(app)
      .put(`/deliveries/checkout/${responseDelivery.body.id}`)
      .attach('signature', path.join(__dirname, 'img/avatar.jpg'))
      .query({
        deliveryman_id: responseDeliveryMan.body.id,
      });

    expect(updatedDeliveryStatus.body).toHaveProperty('end_date');
  });
